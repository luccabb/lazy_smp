...
test_negamax = [
    ...
    (chess.Board("7k/8/8/8/3n4/7q/7P/7K b - - 0 1"), 2, 1, [chess.Move.from_uci('d4f3'), chess.Move.from_uci('h3f1'),  chess.Move.from_uci('d4e2')]),
...
class TestChessFunctions(unittest.TestCase):
    ...
    def test_lazy_smp(self, fen, depth, player, expected_result):
        result = lazy_smp.lazy_smp(fen, depth, player, False)
